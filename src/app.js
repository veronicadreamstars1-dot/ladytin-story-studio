...
import {driveSetupRequirements} from './google-drive.js';
...
function library(title,items,type){
  ...
  return`<section class="panel library" data-library-type="${type}"><div class="panel-head"><div><h2>${title}</h2><p>${items.length} total · ${visible.length} shown</p></div>${badge(type==='main'?'Shared Media':'Shared References')}</div><div class="library-tools"><input id="librarySearch" value="${esc(state.librarySearch)}" placeholder="Search libraries"><select id="libraryFilter"><option value="">All filters</option>${categories.map(category=>`<option value="${esc(category)}" ${category===state.libraryFilter?'selected':''}>${esc(category)}</option>`).join('')}</select><button class="btn tertiary" id="toggleLibraryView">${state.libraryView==='grid'?'List':'Grid'}</button></div><div class="${state.libraryView==='grid'?'library-grid':'library-list'}">${visible.length?visible.slice(0,80).map(item=>{const count=usageCount(item,type);return`<article class="library-card ${item.archived_at?'archived':''}">${preview(item)}<div><b title="${esc(item.filename)}">${esc(item.title||item.filename)}</b><small>${esc(item.filename)} · ${esc(item.type)} · ${count} use${count===1?'':'s'}</small><small>${esc(item.description||item.media_category||'')}</small></div><div class="mini-actions"><button class="btn tertiary" data-show-usage="${type}:${item.id}">Uses</button>${item.google_drive_web_view_link?`<a class="btn tertiary" href="${esc(item.google_drive_web_view_link)}" target="_blank" rel="noopener">Drive</a>`:''}<button class="btn tertiary" data-archive-library="${type}:${item.id}">Archive</button><button class="btn danger" data-delete-library="${type}:${item.id}">Delete</button></div></article>`}).join(''):'<p class="empty-copy">No shared library items yet.</p>'}</div></section>`;
}
...
function wire(){
  ...
  $('#librarySearch')?.addEventListener('input',event=>{state.librarySearch=event.target.value;render()});
  $('#libraryFilter')?.addEventListener('change',event=>{state.libraryFilter=event.target.value;render()});
  $('#toggleLibraryView')?.addEventListener('click',()=>{state.libraryView=state.libraryView==='grid'?'list':'grid';render()});
  ...
  $$('[data-open-source]').forEach(button=>button.onclick=()=>{const id=button.dataset.openSource,item=[...current().mainAssets,...current().references].find(asset=>asset.id===id);if(item?.google_drive_web_view_link)open(item.google_drive_web_view_link,'_blank','noopener');else toast(item?.storage_path?'Stored in private Supabase Storage.':'No source location is available.')});
