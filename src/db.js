...
export function libraryItemToRow(item,{uploadedBy}={}){
  return{id:item.id,library_type:item.library_type,created_by:uploadedBy||item.created_by||null,title:item.title||item.filename||'Untitled item',description:item.description||'',original_filename:item.filename||item.original_filename||'file',mime_type:item.type||item.mime_type||'application/octet-stream',byte_size:item.size||item.byte_size||0,source_type:'supabase_storage',storage_path:item.storage_path||null,thumbnail_path:item.thumbnail_path||null,media_category:item.media_category||'',dominant_colours:item.dominant_colours||[],visual_tags:item.visual_tags||{},search_text:[item.title,item.description,item.filename,item.media_category,(item.tags||[]).join(' ')].filter(Boolean).join(' ')};
}
