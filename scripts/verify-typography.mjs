...
const explicitFiles=['index.html','package.json','vercel.json','.env.example','PINTEREST_REFERENCE_SETUP.md','SUPABASE_DEPLOYMENT.md','supabase/functions/shared-access/index.ts'];
...
async function walk(path){
  ...
  if(!rel.startsWith('tests'))for(const pattern of forbiddenAuthPatterns)if(pattern.test(text))violations.push(`${rel}: removed email-auth mechanism remains (${pattern})`);
  if(rel.startsWith(`dist${process.platform==='win32'?'\\':'/'}`)&&/SUPABASE_SERVICE_ROLE_KEY|PINTEREST_APP_SECRET|PINTEREST_TOKEN_ENCRYPTION_KEY|APP_ACCESS_PASSWORD/i.test(text))violations.push(`${rel}: server-only secret identifier is present in the frontend build`);
}
...
console.log('Security and typography verification passed: monospace UI, password-only anonymous access, no raw password, email-auth code, font binary or server-secret leakage.');
